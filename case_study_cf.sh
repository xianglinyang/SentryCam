...
DATA_PATH="/mnt/hdd1/ljiahao/xianglin/SentryCam"
...
# ---------------------------------------Sub-Cluster Collapse------------------------------------------
MODEL_NAME="ResNet"

LOG_DIR="${DATA_PATH}/${MODEL_NAME}_CIFAR10_SUBCLUSTER_CF"
mkdir -p ${LOG_DIR}
mkdir -p ${LOG_DIR}/config
python -m case_study.catastrophic_forgetting.main --log_dir $LOG_DIR --device cuda:1
python config.py -p ${LOG_DIR}/config/tdvi.yaml
cp case_study/catastrophic_forgetting/models/resnet.py ${LOG_DIR}/Model/model.py
